<script setup lang="ts">
    ...
    import FabricIngredientDialog from '@/views/fabric-ingredient/fabric-ingredient-dialog.vue'
    ...
    myStore.currentPageTitle='Fabric / Fabric Ingredient'
    ...
    const columnDefs=ref([
        ...
        {
            headerName:'Weight/Pcs.',field:'weight',type: 'rightAligned',
        ...
        const dataSource:IDatasource={
            ...
            getRows:async(params:IGetRowsParams)=>{
                try {
                    ...
                    const {data,footer} = await $api<any>(`${import.meta.env.BASE_URL}api/fabric/GetFabricIngredientPartial?request=${JSON.stringify(request)}&exportExcel=false&exportExcel2=false`)
    ...
    const editData=()=>{
        ...
        if(selectedRows.length>0){
            isDialogVisible.value=true
            fabricCode.value=selectedRows[0].fabricCode
        }
    }
    ...
    const exportExcel=async()=>{
        try {
            ...
            const res = await $api<any>(`${import.meta.env.BASE_URL}api/fabric/GetFabricIngredientPartial?request=${JSON.stringify(request)}&exportExcel=true&exportExcel2=false`)
            const url=window.URL.createObjectURL(res)
            const link=document.createElement('a')
            link.href=url
            link.setAttribute('download','FabricIngredient.xlsx')
    ...
    // 👉 Export Excel Version II
    const exportExcel2=async()=>{
        try {
            loadings.value[2]=true
            const request={startRow:0,endRow:0,filterModel:gridApi.value?.getFilterModel(),sortModel:null}
            const res = await $api<any>(`${import.meta.env.BASE_URL}api/fabric/GetFabricIngredientPartial?request=${JSON.stringify(request)}&exportExcel=false&exportExcel2=true`)
            const url=window.URL.createObjectURL(res)
            const link=document.createElement('a')
            link.href=url
            link.setAttribute('download','FabricIngredientVersionII.xlsx')
            document.body.appendChild(link)
            link.click()
            link.remove()
        ...
        <VCardText class="px-1 pt-1 pb-0">
            ...
            <VRow class="my-1">
                <VCol cols="6" class="py-1">
                    ...
                    <VTooltip text="Export Excel Version II">
                        <template v-slot:activator="{props}">
                            <VBtn
                                v-bind="props"
                                class="me-1"
